...
namespace FontTypeRendering {
	...
	void FontTypeRendering::Initialize() {
		...
		if (FT_New_Face(ft_lib, "../Assets/Fonts/Official.ttf", 0, &face) != 0) {
			std::cerr << "Unable to load Official.ttf\n";
			cleanup();
			return exit(-1);
		}
	 ...
	 */
	void FontTypeRendering::render_text(const std::string& str, FT_Face face, float x, float y, float sx, float sy) {
