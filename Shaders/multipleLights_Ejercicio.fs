...
vec3 calculateDirectionalLight(Light light, vec3 direction){
	...
	vec4 colorT0 = texture(texture1, w);
	vec4 colorT1 = texture(texture2, our_uv);
	vec4 totalColor = mix(colorT0, colorT1, 0.3);
...
void main()
{
	...
	vec4 colorFinal = mix(colorText1, colorText2, 0.3);
	if(colorFinal.a < 0.1)
		discard;
    color = vec4(calculateDirectionalLight(directionalLight.light, directionalLight.direction) + calculatePointLights() + calculateSpotLights(), colorFinal.a);
}